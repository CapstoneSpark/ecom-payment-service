...
//
//        // Razorpay header = HEX → must convert to bytes
...
//                    Arrays.asList(objectMapper.readValue(rec.getItemsJson(), OrderItemDto[].class));
//
//            dto.setItems(items);
//
...
//            repository.save(rec);
//
//            log.info("Order pushed to OrderService successfully");
//
//        } catch (Exception ex) {
//            log.error("OrderService push failed", ex);
//        }
//    }
//
//    // ----------------------------------------------------------------------
...
//
//    // ----------------------------------------------------------------------
//    //  SIGNATURE VALIDATION (CORRECT FIX)
//    // ----------------------------------------------------------------------
...
//                if ((productId == null || productId == 0) && sku != null) {
//
...
//                out.setImage(raw.getImage());
//
//                // FIXED subtotal calculation (BigDecimal)
//                BigDecimal subtotal = raw.getUnitPrice()
//                        .multiply(BigDecimal.valueOf(raw.getQuantity()));
...
//
//    // ----------------------------------------------------------------------
//    // PUSH ORDER TO ORDER SERVICE (IDEMPOTENT + 409 SAFE)
//    // ----------------------------------------------------------------------
//    private void pushOrderToOrderService(PaymentRecord rec) {
//
//        try {
...
//                // subtotal FIX
//                BigDecimal subtotal =
//                        raw.getUnitPrice().multiply(BigDecimal.valueOf(raw.getQuantity()));
//
//                out.setSubtotal(subtotal);
//
//                finalItems.add(out);
...
//                log.warn("Order already exists for idempotencyKey {}", rec.getOrderId());
//                rec.setStatus("ORDER_ALREADY_EXISTS");
//                repository.save(rec);
//                return;
//            }
//
...
//    private boolean verifyRazorpaySignature(String payload, String headerHex, String secret) {
//        try {
//            byte[] expected = hexToBytes(headerHex);
//            byte[] computed = hmacSha256(payload, secret);
//            return MessageDigest.isEqual(computed, expected);
//        } catch (Exception ex) {
//            log.error("Signature verification exception", ex);
//            return false;
